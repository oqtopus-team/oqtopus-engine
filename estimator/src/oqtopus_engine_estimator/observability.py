"""OTel observability helpers for oqtopus-engine-estimator.
...
attached upstream by oqtopus-engine core when it opens the per-job root span,
so this enrichment makes TraceQL lookups like ``{ .oqtopus.job_id = "..." }``
...
class JobBaggageSpanProcessor(SpanProcessor):
    ...
    def on_start(  # noqa: PLR6301
        ...
        for key, value in baggage.get_all(ctx).items():
            if key.startswith(_BAGGAGE_PREFIX) and isinstance(
